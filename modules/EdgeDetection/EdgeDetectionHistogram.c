...
// Own header
#include "EdgeDetectionHistogram.h"
...
uint8_t color_cb_min  = 200;
uint8_t color_cb_max  = 255;
uint8_t color_cr_min  = 100;
uint8_t color_cr_max  = 150;

int color_count = 0;


static void send_colorcount(void) {
 DOWNLINK_SEND_COLORCOUNT (DefaultChannel, DefaultDevice, &cnt);
}


void EdgeDetectionHistogram_run(void) {
}
...
void *computervision_thread_main(void* data)
{
  ...
  while (computer_vision_thread_command > 0)
  {
   ...
   grayscale_uyvy(&small, &small);

    //pixel count
   uint8_t *value;
     int cnt = 0;

     for(int y=1; y<small.h*small.w*2;y+=2){
          value=(int) small.buf[y];


          if(value>100){
              small.buf[y+1]=(char)100;
              cnt++;
          }

   }
     register_periodic_telemetry(DefaultPeriodic, "COLORCOUNT", send_colorcount);
...
void EdgeDetectionHistogram_start(void)
{
...
void EdgeDetectionHistogram_stop(void)
{
